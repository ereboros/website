// EREBOROS — gerador dos smartlinks por single
// ---------------------------------------------------------------------------
// Gera uma página /sl/<slug>/index.html para cada single da lista SINGLES,
// a partir de UM template. Assim o Meta Pixel, o GA e o layout ficam num só
// lugar; por single só variam título, capa e links.
//
...
const SINGLES = [
  ...
  {
    slug: "progenies-of-the-unseen",
    title: "Progenies of the Unseen",
    ...
    links: {
      ...
      "apple-music": "https://music.apple.com/us/album/progenies-of-the-unseen-single/6778892074",
      "youtube-music": "https://music.youtube.com/playlist?list=OLAK5uy_k0_tj-UzAPh8NF_OgngO0mhWtAZ_h8UKw",
      "amazon-music": "https://music.amazon.com/albums/B0H4S1Y2TD",
      deezer: "https://link.deezer.com/s/33V4gk4BtSSUMcqtP8W71",
      bandcamp: "https://ereboros.bandcamp.com/track/progenies-of-the-unseen",
    },
  },
  {
...
function renderRow(slug, id, href) {
  ...
  if (href) {
    return `      <a class="sl-link" href="${href}" target="_blank" rel="noreferrer" data-ga-type="listen" data-ga-item="${id}" data-ga-loc="sl-${slug}">
        ...
        <span class="sl-go"><span>Listen</span>${ARROW}</span>
      </a>`;
  }
  return `      <a class="sl-link is-soon" aria-disabled="true">
        ${icon}
        <span class="sl-name">${ic.name}</span>
        <span class="sl-go">Soon</span>
      </a>`;
}
...
<meta property="og:title" content="Ereboros — ${s.title}">
<meta property="og:description" content="${desc}">
<meta property="og:url" content="${url}">
<meta property="og:image" content="${ogImage}">${ogDims}
<meta property="og:image:alt" content="${s.title} — Ereboros">
<meta property="og:locale" content="en_US">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="Ereboros — ${s.title}">
...
<style>
/* ---------- Smartlink (gerado por build-smartlinks.mjs — prefixo .sl-) ---------- */
.sl-shell {
...
.sl-main { flex: 1; padding: clamp(16px, 3.5vh, 36px) 0 clamp(28px, 5vh, 48px); }

/* Cabeçalho */
.sl-head { text-align: center; margin-bottom: clamp(20px, 3.5vh, 32px); }
.sl-kicker {
  font-family: var(--f-mono); font-size: 12px; letter-spacing: 0.3em;
  text-transform: uppercase; color: var(--oxide-bright); margin: 0 0 12px;
}
.sl-title {
  font-family: var(--f-display); font-weight: 600; color: var(--bone);
  font-size: clamp(28px, 6.2vw, 46px); line-height: 1.04; margin: 0;
}
...
.sl-empty {
  width: min(100%, 460px); margin: 0 auto clamp(24px, 4vh, 38px);
  aspect-ratio: 1 / 1; border: 1px solid var(--rule-strong); background: var(--ink-2);
  box-shadow: 0 26px 70px rgba(0,0,0,.55);
  display: grid; place-items: center; align-content: center; gap: 16px; padding: 24px;
}
.sl-empty img { width: 52%; max-width: 220px; height: auto; opacity: .4; }
.sl-empty span {
...
.sl-links { display: flex; flex-direction: column; gap: 12px; }
.sl-link {
  display: flex; align-items: center; gap: 16px;
  padding: 16px 20px; border: 1px solid var(--rule-strong); background: var(--ink-2);
  transition: border-color .2s, background .2s, transform .12s;
}
.sl-link:hover { border-color: var(--oxide-bright); background: var(--ink-3); }
.sl-link:active { transform: scale(.99); }
.sl-link .sl-ico { flex: none; width: 26px; height: 26px; display: block; color: var(--bone); }
.sl-link .sl-name {
...
.sl-link .sl-go {
  ...
  text-transform: uppercase; color: var(--bone-dim); transition: color .2s;
}
.sl-link:hover .sl-go { color: var(--bone); }
.sl-link .sl-go svg { transition: transform .2s; }
.sl-link:hover .sl-go svg { transform: translate(2px, -2px); }

/* Estado "em breve": botão travado, sem link */
.sl-link.is-soon { cursor: default; opacity: .7; border-color: var(--rule); }
.sl-link.is-soon:hover, .sl-link.is-soon:active {
  border-color: var(--rule); background: var(--ink-2); transform: none;
}
.sl-link.is-soon .sl-ico, .sl-link.is-soon .sl-name { color: var(--bone-dim); }
.sl-link.is-soon .sl-go {
  color: var(--ash-2); border: 1px solid var(--rule);
  padding: 3px 11px; border-radius: 999px; letter-spacing: 0.2em;
}
...
.sl-foot .meta a { border-bottom: 1px solid var(--rule); }
.sl-foot .meta a:hover { color: var(--bone); border-color: var(--rule-strong); }

/* Indicador de rolagem: só aparece quando há conteúdo abaixo da dobra (ver script) */
.sl-scrolldown {
  position: fixed; left: 50%; bottom: 16px; transform: translateX(-50%);
  z-index: 5; display: grid; place-items: center; width: 40px; height: 40px;
  padding: 0; border: 1px solid var(--rule-strong); border-radius: 999px;
  background: rgba(10,9,8,.6); color: var(--bone-dim); cursor: pointer;
  -webkit-backdrop-filter: blur(3px); backdrop-filter: blur(3px);
  animation: sl-bounce 1.8s ease-in-out infinite;
  transition: opacity .35s, color .2s, border-color .2s;
}
.sl-scrolldown:hover { color: var(--bone); border-color: var(--oxide-bright); }
.sl-scrolldown svg { display: block; width: 20px; height: 20px; }
.sl-scrolldown.is-hidden { opacity: 0; pointer-events: none; }
@keyframes sl-bounce {
  0%, 100% { transform: translate(-50%, 0); }
  50%      { transform: translate(-50%, 5px); }
}
@media (prefers-reduced-motion: reduce) { .sl-scrolldown { animation: none; } }

@media (max-width: 520px) {
  .sl-link .sl-go span { display: none; } /* no mobile deixa só a seta, menos poluído */
  .sl-link { padding: 15px 18px; }
}
</style>
</head>
<body>
...
<div class="sl-shell">
  ...
  <main class="sl-main">
    ...
    <div class="sl-head">
      <p class="sl-kicker">New single · Ereboros</p>
      <h1 class="sl-title">${s.title}</h1>
    </div>
...
${rows}
  ...
  <footer class="sl-foot">
    ...
    <span class="meta"><a href="/" data-ga-type="navigate" data-ga-item="home-footer" data-ga-loc="sl-${s.slug}">ereboros.com</a> · <a href="mailto:contact@ereboros.com">contact@ereboros.com</a></span>
  </footer>

  <button type="button" class="sl-scrolldown is-hidden" aria-label="Rolar para ver as plataformas">
...
document.addEventListener("click", function (e) {
  ...
  if (!el) return;
  var type = el.getAttribute("data-ga-type");
  var item = el.getAttribute("data-ga-item") || "";
  var loc  = el.getAttribute("data-ga-loc") || "";
...
// Indicador de rolagem: aparece só se há conteúdo abaixo da dobra; some ao chegar no fim.
// Clicar rola suave até a lista de plataformas (revela os botões no tráfego de ad).
(function () {
  var el = document.querySelector(".sl-scrolldown");
  if (!el) return;
  var links = document.querySelector(".sl-links");
  el.addEventListener("click", function () {
    (links || document.body).scrollIntoView({ behavior: "smooth", block: "start" });
  });
  function update() {
    var scrollable = document.documentElement.scrollHeight - window.innerHeight;
    var nearBottom = window.scrollY >= scrollable - 24;
    el.classList.toggle("is-hidden", scrollable <= 24 || nearBottom);
  }
