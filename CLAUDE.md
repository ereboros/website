...
**Cache-busting.** `index.html` referencia os assets com query de versão (atualmente `?v=27`): `styles.css`, `data.js`, `components.jsx`, `sections.jsx`, `app.jsx`. **Ao editar qualquer um desses arquivos, incremente o número de versão** (de preferência todos juntos) para invalidar o cache do navegador — sem isso, o Babel/CSS pode rodar a versão antiga em cache, inclusive para visitantes recorrentes após o deploy.
