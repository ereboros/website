...
Por single só variam: `title`, `slug`, `cover`/`coverOg` (capa; `cover: null` mostra o placeholder "Artwork coming soon" e usa `band-promo.jpg` como og:image), `links` (`null` = botões travados com badge "Soon"; objeto com as URLs = botões ativos "Listen" mais evento de conversão) e `releaseDate` (rótulos de data e JSON-LD). Há também um CTA opcional de pré-save, que sai sozinho da página quando `links` é preenchido. **A capa não linka o clipe, de propósito** (decisão de 04.09.2026, explicada no topo do `.mjs`): o smartlink tem um único trabalho, levar ao streaming. O clique de streaming (e o de pré-save) dispara **`Lead` e `ViewContent`** no Pixel, os dois com o mesmo significado: o `ViewContent` é o evento que a campanha em curso otimiza, e mantê-lo evita reiniciar o aprendizado. Nada mais dispara evento padrão, para a campanha não ter um alvo mais fácil que o clique de streaming. A ordem dos botões vem da constante `ORDER`, e os ícones são paths SVG embutidos no mesmo arquivo.
